<template>
	<view v-if="bgCoverImg" class="content" :style="{'background-image': `url(${bgCoverImg})`}">
		...
		<view  class="footer-btn">
			<button type="default" @click="tossCoin()">抛硬币</button>
		</view>
	...
	export default {
		...
		methods: {
			...
			getBgImg() {
				...
				this.bgCoverImg = `https://cdn.jsdelivr.net/gh/unilei/cdn@2.1/bg/${bgSrc}.jpg`;
				this.bgCoverImg = `https://cdn.jsdelivr.net/gh/unilei/cdn@2.1/bg/${bgSrc}.jpg`;
			},
			// 获取硬币图片
			getCoinImg() {
				let name = uni.getStorageSync('coinName') || '2020shu'
				this.coinImgFront = `https://cdn.jsdelivr.net/gh/unilei/cdn@2.1/coin/${name}_front.png`
				this.coinImgBack = `https://cdn.jsdelivr.net/gh/unilei/cdn@2.1/coin/${name}_back.png`
			},
			// 获取硬币旋转音频
			getAudio() {
...
<style lang="scss">
	...
	$coin-diameter: 200rpx; //直径
	$coin-thickness: 16rpx; // 厚度
	$turn-time: 1s; // 转动时间
